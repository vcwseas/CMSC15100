...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname proj2-seed) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
...
(define (possible-moves g)
  ...
  [(game next inv board)
    (if (ormap (λ (p) (victory? p g)) '(blue orange))
      '()
      (append (possible-intros g) (possible-shifts g)))]))

      
;; possible-intros: game -> (listof move)
;; returns all the possible intros for the current player
;; tests are included in later section. 
      
(define (possible-intros g)
  (match g
    [(game next inv board)
     (local { (define next-inv (if (equal? next 'blue) (pair-snd inv) (pair-fst inv)))
              (define squares (build-list 9 identity))
              (define (pi rest-inv lom)
                (cond
                  [(empty? rest-inv) lom]
                  [else 
                   (pi (cdr rest-inv)
                       (foldl (λ (m lst) (if (not (false? m)) 
                                             ...
                                             lst))
                              lom
                              (map (λ (n) (if (move-legal? (make-intro (car rest-inv) n) g)
                                              (make-intro (car rest-inv) n)
                                              #f)) squares)))])) }
       (pi next-inv empty))]))


;; possible-shifts: game -> (listof move)
;; returns all the possible shifts for the current player
;; tests are included in later section.
(define (possible-shifts g)
  (match g
      [(game next inv board)
       (local { (define squares (build-list 9 identity))
                (define lop (foldl (λ (n acc) (if (and (not (empty? (list-ref board n)))
                                                        (equal? (piece-player (car (list-ref board n))) next))
                                                  (cons n acc)
                                                  acc)) empty squares))
                (define (ps rest-inv lom)
                  (cond
                    [(empty? rest-inv) lom]
                    [else
                     (ps (cdr rest-inv) 
                         (foldl (λ (m lst) (if (not (false? m))
                                           ...
                                           lst))
                            lom
                            (map (λ (n) (if (move-legal? (make-shft (car rest-inv) n ) g)
                                            (make-shft (car rest-inv) n)
                                            #f)) squares)))])) }
         (ps lop empty))]))
...
(check-expect (possible-intros test-game) (list
 (make-intro (make-piece 3 'blue) 8)
 (make-intro (make-piece 3 'blue) 7)
 (make-intro (make-piece 3 'blue) 6)
 (make-intro (make-piece 3 'blue) 4)
 (make-intro (make-piece 3 'blue) 3)
 (make-intro (make-piece 3 'blue) 1)
 (make-intro (make-piece 3 'blue) 0)
 (make-intro (make-piece 2 'blue) 8)
 (make-intro (make-piece 2 'blue) 7)
 (make-intro (make-piece 2 'blue) 6)
 (make-intro (make-piece 2 'blue) 4)
 (make-intro (make-piece 2 'blue) 1)
 (make-intro (make-piece 1 'blue) 7)
 (make-intro (make-piece 1 'blue) 4) 
 (make-intro (make-piece 1 'blue) 1)))
