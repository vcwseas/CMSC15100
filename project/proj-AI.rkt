...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname proj-AI) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
...
;; AI-move: game -> move
;; uses game tree to decide on move
;; depth of 3
(define (AI-game g)
  ( local { (define lvl-1 (possible-moves g)) 
            (define (win-move 



;; have to define own apply-move function that creates a game after appying a move

(define (win-move lom)
  (if (empty? lom)
      #f
      (if (victory? (apply-move(car lom))
          (car lom)
          (win-move (cdr lom)))))
...
(define (max-val-pair lop)
  (foldl (λ (p1 p2) (if (> (pair-snd p1) (pair-snd p2))
                        p1
                        p2)) (car lop) lop))
