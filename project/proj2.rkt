  ...
  (match g
    [(game next inv board)
     (local { (define squares (build-list 9 identity))
              ;; next-inv is (or (listof num) (listof pieces)) depending on the type of move given. 
              ...
              ;; pm : (listof piece) (listof moves) -> (listof moves)
              ;; tests if a move of either type is valid, then adds it to 
              ;; an aggregate list of possible moves
...
(define victory-game (make-game 'blue
                                (make-pair
                                 (list orange1 orange2 orange3)
                                 (list blue1 blue2 blue3))
                                (list (list blue1)
...
(check-expect (possible-moves victory-game) '())
(check-expect (member (make-shft 2 0) (possible-moves test-game)) true)
(check-expect (member (make-intro blue1 1) (possible-moves test-game)) true)
(check-expect (member (make-intro orange3 1) (possible-moves test-game)) false)
(check-expect (member (make-intro blue3 2) (possible-moves test-game)) false)


(check-expect (length (possible test-game make-shft)) 15)
(check-expect (member (make-shft 8 7) (possible test-game make-shft)) true)
(check-expect (length (possible test-game make-intro)) 15)
(check-expect (member (make-intro blue1 7) (possible test-game make-intro)) true)



(check-expect (random-move victory-game) false)
;; testing that random-moves produce only valid moves
