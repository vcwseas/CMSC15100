...
;; Ho Yin Cheung 438907 CS151 Aut 13 John Reppy
...
;; NOTE: Minimax-driven AI becomes self-defeating with ply higher than 2.
;; This is because the AI thinks that the player has already won, 
;; whereas the player may not realize this. The AI then decides that
;; every possible move is equivalent because each leads to inevitable
;; defeat. The AI then chooses moves that seem random to the player.
;; This issue was not addressed by the assignment given and I make
;; no attempt to resolve it here. 
...
(define (apply-move g m)
  (local { (define other (if (equal? (game-next g) 'blue) 'orange 'blue)) }
  (match m
    [(intro p sq) (local { (define new-game (apply-drop p sq (apply-intro-lift p g))) }
                    (make-game other (game-inv new-game) (game-board new-game)))]
    [(shft src dst) (local { (define new-game (match (apply-shft-lift src g)
                                                 [(pair p tmp-g)
                                                  (if (victory? other tmp-g)
                                                      (make-game other (game-next tmp-g) 
                                                                 (game-board tmp-g))
...
(check-expect (apply-move (new-game 'blue) (make-intro orange1 0)) 
              (make-game 'orange (make-pair (list orange1 orange2 
                                                  orange2 orange3
                                                  orange3)
                                            (list blue1 blue1 blue2
                                                  blue2 blue3 blue3))
                         (list 
                          (list orange1) empty empty
                          empty empty empty 
                          empty empty empty)))
(check-expect (apply-move (make-game 'orange (make-pair (list orange1 orange2 
                                                              orange2 orange3
                                                              orange3)
                                                        (list blue1 blue1 blue2
                                                              blue2 blue3 blue3))
                                     (list 
                                      ...
                                      empty empty empty))
                          (make-shft 0 1))
              (make-game 'blue (make-pair (list orange1 orange2 
                                                              orange2 orange3
                                                              orange3)
                                                        (list blue1 blue1 blue2
                                                              blue2 blue3 blue3))
                                     (list 
                                      empty (list orange1) empty
                                      empty empty empty 
                                      empty empty empty)))
...
(check-expect (score (new-game 'orange)) 0)
(check-expect (score (make-game 'blue (make-pair '() '()) (list 
                                                           (list orange1) 
                                                           (list orange2) 
                                                           (list orange3)
                                                           empty empty empty
...
(define (minimax ply g)
  (local {(define player (game-next g))
          (define select (if (symbol=? player 'blue) min max))
          (define (traverse i g)
             (cond
               [(or (either-victory? g) (>= i ply)) (score g)]
                [else (apply select (map (λ (new-game) (traverse (add1 i) new-game)) 
                                                        (map (λ (m) (apply-move g m)) 
                                                             (possible-moves g))))]))}
    (traverse 0 g)))
(check-expect (minimax 3 (new-game 'blue)) 0)
(check-expect (minimax 3 (make-game 'blue (make-pair '() '()) (list 
                                                           (list orange1) 
                                                           (list orange2) 
                                                           (list orange3)
                                                           empty empty empty
...
(define (choose-move-aux ply g tt?)
  (local { (define next-moves (possible-moves g))
           ...
           ;; list of scores for next states
           ...
           ;; find the indexes of all elements equal to itme in a list
    ...
    ;; displays the list of scores along with their respective moves
    (begin (display los) (display (map move->string next-moves))
      ...
      (if (either-victory? g) 
          (error "game already won") 
           (list-ref next-moves 
                     ;; change car to random-item for random-moves.
...
(check-expect (h-v (make-game 'orange (make-pair '() '()) (list
                                                           empty empty (list blue3)
                                                           empty empty (list orange2)
                                                           empty empty empty)))
              2199027449856)
(check-expect (h-v (new-game 'orange)) 0)
...
(check-expect (s->val (list orange3 orange2 orange1)) 42)
...
(define (find lot h)
  (match lot
    ...
    [(cons x xs) (if (= (tt-entry-hash x) h)
                     x
                     (find xs h))]))
(check-expect (find (list (make-tt-entry 12412142 1 1)
                          (make-tt-entry 12312412 1 0))
                    12312412)
              (make-tt-entry 12312412 1 0))
(check-expect (find (list (make-tt-entry 12412142 1 1)
                          (make-tt-entry 12312412 1 0))
...
;; TT-find: (listof hash) -> (listof tt-entry)
;; given a list of hashes, finds all existing tt-entries 
;; within TT
;; manually tested
(define (TT-find loh)
  (local { (define (aux loh acc) 
             (if (empty? loh) 
                 acc
                 (match (modulo (car loh) TT-size)
                   [i 
                    (match (find (vector-ref TT i) (car loh))
                                 [#f (aux (cdr loh) acc)]
                      [entry (aux (cdr loh) (cons entry acc))])]))) }
    (aux loh '())))
...
;; TT-remove!: hash num-> void
;; removes a game with a certain hash from a list of tt-entries
;; manually tested.
(define (TT-remove! h)
  (match (modulo h TT-size)
  [i (vector-set! TT i
               (remove (find (vector-ref TT i) h) (vector-ref TT i)))]))
...
(check-expect (either-victory? (make-game 'blue (make-pair '() '()) (list 
                                                           (list orange1) 
                                                           (list orange2) 
                                                           (list orange3)
                                                           empty empty empty
...
(check-expect (either-victory? (new-game 'orange)) false)
(check-expect (either-victory? (make-game 'blue 
                                          (make-pair (list orange1 orange2 orange3)
                                                           (list blue1 blue2 blue3))
                                          (list 
                                           (list blue1) (list orange1) (list blue2)
                                           (list blue3) (list orange2) empty
                                           empty (list orange3) empty))) true)
...
(check-expect (member 17473 (h-equiv 
                             (make-game 'blue (make-pair '() '()) (list 
                                                           (list orange1) 
                                                           (list orange2) 
                                                           (list orange3)
                                                           empty empty empty
...
;; given max ply and game, return score using
;; minimax search with the transposition table
;; manually tested
...
;; of a given state
;; auxilary to minimax-tt, tested with minimax-tt
...
(define (TT-match player i g)
  (local { (define (find loh inverted?)
             (local { (define matching-entries (TT-find loh)) }
               (recur check-entries ([loe matching-entries])
                        (if (empty? loe)
                            #f
                            (match (car loe)
                              [(tt-entry h s ply) 
                               (if (tt-entry-valid ply i s)
                                   (if inverted? (- s) s)
                                   (check-entries (cdr loe)))]))))) }
    
    (match (find (h-equiv g false) false)
      [#f 
       (match (find (h-equiv g true) true)
         ;; traverse-insert is called only if existing tt-entries
         ;; are invalid or if no entry exists. 
...
;; auxilary to minimax-tt, 
;; tested with minimax-tt
(define (traverse-insert player i g)
               (local { (define score 
                                 (apply (if (equal? player 'blue) min max) 
                                        ;; list of scores representing each game state
                        ...
                        (begin
                          (match (h-v g)
                            [h
                             (begin
                             ;; remove existing entries that cover the same game.
                             ;; because traverse-insert is only called when
                             ;; - existing entry is invalid (shallower ply)
                             ;; - no entry exists
                             ;; then remove will always prevent duplicates. 
                             (TT-remove! h)
                             (TT-insert! h score i))])
                               score)))
...
(define (tt-entry-valid p i s)
  (or (>= p i) 
      (or (= 1 s)
          (= -1 s))))
(check-expect (tt-entry-valid 3 2 1) true)
(check-expect (tt-entry-valid 2 3 0) false)
...
(define (run-mm ply human-player)
  ((λ (x) true)
   ;; ply = 0 means that the computer chooses randomly from possible moves
   (run-game human-player (if (= ply 0) random-move (AI-move ply)))))
...
(define (run-mm-tt ply human-player)
  ((λ (x) true)
   ;; ply = 0 means that the computer chooses randomly from possible moves 
   (run-game human-player (if (= ply 0) random-move (tt-move ply)))))
