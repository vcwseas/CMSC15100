...
;; #grader: Contracts and purposes need to precede their functions, not tests.
(define concat-with
  (λ (s lst)
    (if (empty? lst) 
        '()
...
;; === problem 1
;(check-expect (concat-with "," empty) "")
(check-expect (concat-with "," (list "a")) "a")
(check-expect (concat-with "," (list "a" "b")) "a,b")
(check-expect (concat-with "," (list "a" "b" "c")) "a,b,c")

;; === problem 2
(chessboard 1 20 "orange" "black")
(chessboard 2 15 "orange" "black")
(chessboard 3 15 "orange" "black")
(chessboard 8 12 "orange" "black")
(chessboard 8 20 "gray" "maroon")
(chessboard 20 4 "gray" "maroon")
...
;; === problem 4
(check-expect (full-linreg (list (make-posn 0 0) (make-posn 1 1)))
              (make-analysis (make-lineq 1 0) 1))
(check-within (full-linreg (list (make-posn 1 1.1) 
                                 (make-posn 2 1.9)
                                 (make-posn 3 3.3)))
              (make-analysis (make-lineq 1.1 -0.1) 0.97580645)
              0.00001)

;; === problem 5
(recursive-tiles 16)
(recursive-tiles 32)
(check-expect (image-width (recursive-tiles 64)) 64)
(time (recursive-tiles 512))
...
;; contracts                         6/ 8 (Contracts and purposes need to precede their functions, not tests.)
;; well-written purposes             6/ 8 (Same as above.)
;; adequate tests                    8/ 8
;; svn usage                         2/ 2
;; _subtotal_                       46/50
