...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname graded-hw1) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
...
(define weekly-pay
  (lambda (wage hours)
      (if (<= 0 (overtime hours)) 
        ...
        (* (- 40 hours) wage))))
;; #grader: no need for lambdas in any of these functions
;; #grader: incorrectly computes wage when under 40 hours worked (except for 20. try 1 or 21)
...
;; #grader: move the similar code in your flag functions to a helper function to avoid duplication
...
;; #grader: incorrectly reports true when images are the same size
...
;; === problem 1
;; #grader: Ok
;(check-expect (lbkgconvert (kglbconvert 111)) 111)
;(check-expect (kglbconvert (lbkgconvert 111)) 111)

;(check-expect (mikmconvert (kmmiconvert 111)) 111)
;(check-expect (kmmiconvert (mikmconvert 111)) 111)

(check-expect (fcconvert (cfconvert 111)) 111)
(check-expect (cfconvert (fcconvert 111)) 111)

;; === problem 2
(check-expect (pythagorean? 3 4 5) true)
(check-expect (pythagorean? 4 3 5) true)
(check-expect (pythagorean? 4 4 5) false)

;; === problem 3
(check-expect (weekly-pay 10 1) 10)
(check-expect (weekly-pay 10 40) 400)
(check-expect (weekly-pay 10 41) 415)
...
(check-expect (cab-fare-with-tip 50) 58)

;; === problem 5
(check-expect (num-of-digits 1) 1)
(check-expect (num-of-digits 2) 1)
(check-expect (num-of-digits 3) 2)
(check-expect (num-of-digits 4) 3)
(check-expect (num-of-digits 9999) 39996)

;; === problem 6
(check-within (hemishell-volume 2 1) (* 14/3 pi) 0.0000001)
(check-within (hemishell-volume 3 2) (* 52/3 pi) 0.0000001)
(check-within (hemishell-volume 3 1) (* 38/3 pi) 0.0000001)

(check-within (hemishell-area 2 1) (* 13 pi) 0.0000001)
(check-within (hemishell-area 3 2) (* 28 pi) 0.0000001)
(check-within (hemishell-area 3 1) (* 31 pi) 0.0000001)
...
(check-expect (wider? (square 10 "solid" "blue") (square 9 "solid" "blue")) true)
(check-expect (wider? (square  9 "solid" "blue") (square 9 "solid" "blue")) false)
(check-expect (wider? (square  8 "solid" "blue") (square 9 "solid" "blue")) false)

(check-expect (narrower? (square 10 "solid" "blue") (square 9 "solid" "blue")) false)
(check-expect (narrower? (square  9 "solid" "blue") (square 9 "solid" "blue")) false)
(check-expect (narrower? (square  8 "solid" "blue") (square 9 "solid" "blue")) true)

;; === grading ===

;; #grader: you don't need to use lambdas when defining functions

;; === correctness ===
;; problem 1 (conversions)           6/ 6
;; problem 2 (pythagorean)           3/ 3
;; problem 3 (wages)                 1/ 3
;; problem 4 (cab)                   3/ 3
;; problem 5 (d)                     4/ 4
;; problem 6 (volume)                3/ 3
;; problem 6 (surface area)          3/ 3
;; problem 7 (flags)                 8/ 8
;; problem 8 (img width)             2/ 3
...
;; program decomposition (helpers)   6/ 8
;; contracts                         8/ 8
;; well-written purposes             8/ 8
;; adequate tests                    6/ 8 several functions with only 1 test
